...
# from selfAttention import SelfAttentionModule
# from fusionGate import GatedFusionModule
import torch
            ...
            while c_begin < self.sizes[2]:
                ...
                while b_begin < len(queries):
                    ...
                    for i, query in enumerate(these_queries):
                        ...
                        # scores[i, torch.LongTensor(self.property_id)] = -1e6 # todo remove property_id
...
class FSTKGC(TKBCModel):
    ...
    def __init__(self, sizes: Tuple[int, int, int, int], rank: int,no_time_emb=False, init_size: float = 1e-2):
        ...
        # todo fusuion static
        # df = pd.read_csv("data/100_envAform/only2static.tsv", sep="\t", header=None, names=["key", "value"])
        # self.static_ent_index = df.set_index("key")["value"].to_dict() # 读取paleo实体到static实体的映射 形式：{'paleo_id': static_id}
        # self.static_ent_emb = torch.load('data/hierarchy/hierarchy_embeddings.pth') # 读取实体emb
        # end todo
    ...
    def forward(self, x):
        ...
        # # todo
        # static_emb = lhs.clone()
        # for i, e in enumerate(lhs_ids):
        #     if int(e) in self.static_ent_index:
        #         static_emb[i] = self.static_ent_emb[self.static_ent_index[int(e)]]
        # l_mse = nn.MSELoss()(lhs.float(), static_emb.float())
        # # l_mse = nn.L1Loss()(lhs.float(), static_emb.float())
        # # l_mse = nn.MSELoss()(lhs, static_emb)
        # # end todo
